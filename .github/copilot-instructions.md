...
- Treat source kind and artifact kind as separate concepts in task logic and docs. Infer whether a repository is a PowerShell module from a valid source manifest, then use explicit artifact context (for example `Chocolatey`) when the same source tree produces a different artifact type.
