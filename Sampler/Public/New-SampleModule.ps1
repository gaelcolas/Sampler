...
#     # Determine the template path based on the module type
...
#     # Parameters that should not be passed to Plaster (they're used by New-SampleModule only)
#     $excludeFromPlaster = @('ModuleType', 'DestinationPath', 'ModuleName', 'Features')

#     foreach ($paramName in $MyInvocation.MyCommand.Parameters.Keys)
#     {
#         $paramValue = Get-Variable -Name $paramName -ValueOnly -ErrorAction SilentlyContinue

#         # Skip ModuleType for DscV3Module template as it doesn't have this parameter
#         if ($ModuleType -eq 'DscV3Module' -and $paramName -in $excludeFromPlaster)
#         {
#             continue
#         }
...
function New-SampleModule
{
    ...
    foreach ($paramName in $MyInvocation.MyCommand.Parameters.Keys)
    {
        ...
        if ($paramName -in $excludeFromPlaster)
        {
            continue
        }
