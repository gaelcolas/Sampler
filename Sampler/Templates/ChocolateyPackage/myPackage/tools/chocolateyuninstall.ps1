...
{
    Write-Warning -Message "$($key.Count) matches found!"
    Write-Warning -Message "To prevent accidental data loss, no programs will be uninstalled."
    Write-Warning -Message "Please alert package maintainer the following keys were matched:"
    $key | ForEach-Object -Process {
      Write-Warning -Message "- $($_.DisplayName)"
    }
}
